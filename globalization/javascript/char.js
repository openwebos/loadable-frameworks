...
/*globals console Globalization */

Globalization.Character = Globalization.Character || {};

/*$ private */
Globalization.Character._isIdeographLetter = function (num)
{
...
/*$ private */
Globalization.Character._isIdeographOther = function (num)
{
...
/**
 * Globalization.Character.isIdeograph(ch) -> Object
 * - ch (String): character being queried 
 *
 * Return true is the first character in the string an Asian
 * ideographic character. 
 * 
 * Returns true if the first character in the string is an 
 * Asian ideographic character. 
 **/
Globalization.Character.isIdeograph = function (ch) {
	var num;
	...
	return Globalization.Character._isIdeographLetter(num) || Globalization.Character._isIdeographOther(num);
};

// taken from http://www.unicode.org/Public/5.2.0/ucd/PropList.txt
Globalization.Character._punctChars = {
...
/**
 * Globalization.Character.isPunct(ch) -> Object
 * - ch (String): character being queried 
 *
 * Return true is the first character in the string a 
 * punctuation character. 
 * 
 * Returns true if the first character in the string is a 
 * punctuation character. 
 **/
Globalization.Character.isPunct = function (ch) {
	var num;
	...
	num = ch.charCodeAt(0);
	return (num in Globalization.Character._punctChars);
};

// taken from http://www.unicode.org/Public/5.2.0/ucd/PropList.txt
//and now, spacechars, the movie
Globalization.Character._spaceChars = {
	0x0009: 1,	// tab
	0x000A: 1,	// line feed
...
/**
 * Globalization.Character.isWhitespace(ch) -> Object
 * - ch (String): character being queried 
 *
 * Return true is the first character in the string a 
 ...
 * whitespace character. 
 **/
Globalization.Character.isWhitespace = function (ch) {
	var num;
	if ( !ch || ch.length < 1 ) {
		return false;
	}
	...
	num = ch.charCodeAt(0);
	return (num in Globalization.Character._spaceChars);
};

/**
 * Globalization.Character.toUpperCase(str, locale) -> String
 ...
 *
 * Upper-case every character in a string according to the rules of the 
 * given locale. If the locale is not given, the current locale is
 * used.
 *  
 * Returns a string containing the same content as the original parameter, but with
 * all characters upper-cased
 **/
Globalization.Character.toUpperCase = function (str, locale) {
	var loc, langinfo;
	...
	loc = Globalization.Locale.parseLocaleString(locale);
	langinfo = Globalization.Locale._loadLanguageInfo(loc.language);
	...
	if ( langinfo && langinfo.upperMap !== undefined ) {
		return Globalization.Locale._stringTransform(str, langinfo.upperMap);
	}
...
/**
 * Globalization.Character.isLetter(ch) -> Object
 * - ch (String): character being queried
 *
 * Return true is the first character in the string a
 * letter character. 
 ...
 * letter character. 
 **/
Globalization.Character.isLetter = function (ch) {
	...
	letterChars = Globalization.Locale._loadLanguageInfo('chartype.letter');
	
	ret = (letterChars && (c in letterChars)) || Globalization.Character._isIdeographLetter(num);
